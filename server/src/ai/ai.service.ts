...
class AIService implements AIProvider {
  ...
  constructor() {
    this.provider = createAIProvider({
      provider: 'local',
      apiToken: '', // Not needed for local offline models
      chatModel: env.LOCAL_CHAT_MODEL,
      embeddingModel: env.LOCAL_EMBEDDING_MODEL,
    });
  }

  async chatCompletion(params: ChatCompletionParams): Promise<string> {
    logger.info({ model: env.LOCAL_CHAT_MODEL }, 'Calling chatCompletion');
  ...
  async *chatCompletionStream(params: ChatCompletionParams): AsyncIterable<string> {
    logger.info({ model: env.LOCAL_CHAT_MODEL }, 'Calling chatCompletionStream');
  ...
  async generateEmbedding(text: string): Promise<number[]> {
    logger.info({ model: env.LOCAL_EMBEDDING_MODEL, length: text.length }, 'Calling generateEmbedding');
    return this.withRetry(() => this.provider.generateEmbedding(text));
  }

  async generateEmbeddings(texts: string[]): Promise<number[][]> {
    logger.info(
      { model: env.LOCAL_EMBEDDING_MODEL, count: texts.length },
