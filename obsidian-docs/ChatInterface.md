...
- If `!isReady`: renders a "Chat Disabled" placeholder (no `useChat` data shown) explaining processing must finish first.
...
## Dependencies
- Imports: [[useChat]] (`@/hooks/useChat`), [[ChatMessage]], [[ChatInput]], shadcn `Button`, `cn`.
